...
class Village:
    ...
    def town_planner(self):
        ...
        roads = set()
        allocated = set()
        available = set(filter(self.grid.is_3x3, self.grid.adjacency_list))
        ...
        available_water = {tile for tile in available if tile.terrain == Biome.WATER}
        ...
        for i in range(max_houses):
            if len(available_ground) > 0:
                ...
                allocated = allocated.union(self.grid.get_square(plot, size=3))
                plots.add(plot)
                land_houses += 1
            else:
                if land_houses + water_houses < min_houses:
                    if len(available_water) > 0:
                        ...
                        break
        post(f"building {land_houses} on ground, {water_houses} on water")

        possible_roads = set(self.grid.adjacency_list.keys()).difference(allocated)
        houses_to_build = set()
        ...
        available_for_misc = (available_for_misc.difference(roads)).difference(
            allocated
        )
    ...
    def place_lights(self, roads):
        ...
        for tile in lantern_tiles:
            ...
            if get_block(a.x, a.y + 1, a.z) == block.AIR.id:
                set_blocks(
                    a.x, a.y + 1, a.z, a.x, a.y + 3, a.z, block.FENCE_DARK_OAK
                )  # Cobblestone wall
    ...
    def place_misc(self, available):
        ...
        for i in range(num_misc):
            ...
            for tile in available:
                distance = abs(tile.position.x - center.position.x) + abs(
                    tile.position.z - center.position.z
                )
    ...
    def build_tent(tile):
        ...
        if tile.terrain == Biome.WATER:
            set_blocks(
                a.x - 2, a.y - 3, a.z - 2, a.x + 2, a.y, a.z + 2, block.WOOD_PLANKS.id
            )
        else:
            set_blocks(
                a.x - 2, a.y - 3, a.z - 2, a.x + 2, a.y, a.z + 2, block.COBBLESTONE.id
            )
            ...
            for j in range(5):
                if direction == 1:
                    if j % 2 == 1:
                        ...
                        colour = colour2
                if (
                    get_block(a.x - 2 + i, a.y + 4, a.z - 2 + j) == block.AIR.id
        ...
        for i in range(random.randint(0, 6)):
            ...
            set_block(
                a.x + random.randint(-1, 1),
                a.y + 1,
                a.z + random.randint(-1, 1),
                goods_type,
            )
    ...
    def get_door_vec(center, door):
        ...
        d = door.position
        if c.x > d.x:
            return Vec3(c.x - 4, c.y, c.z)
        elif c.x < d.x:
            return Vec3(c.x + 4, c.y, c.z)
        elif c.z < d.z:
            return Vec3(c.x, c.y, c.z + 4)
        else:
            return Vec3(c.x, c.y, c.z - 4)
    ...
    def _place_road_square(a, block_type, random_type):
        set_blocks(a.x - 1, a.y - 1, a.z - 1, a.x + 1, a.y, a.z + 1, block_type)
        ...
        if random_type is not None:
            for i in range(3):
                for j in range(3):
                    if random.randint(1, 4) == 1:
                        set_block(
                            a.x - 1 + i, a.y, a.z - 1 + j, random.choice(random_type)
                        )

        set_blocks(a.x - 1, a.y + 1, a.z - 1, a.x + 1, a.y + 30, a.z + 1, block.AIR.id)
    ...
    def _place_road_connector(a, b, block_type, second):
        ...
        if a.x != b.x:
            if a.x < b.x:
                ...
                set_blocks(
                    a.x + 2,
                    a.y + height_difference + 1,
                    a.z - 1,
                    a.x + 2,
                    a.y + height_difference + 30,
                    a.z + 1,
                    block.AIR.id,
                )
                if second:
                    set_blocks(
                        a.x + 2, a.y - 10, a.z + 2, a.x + 2, a.y, a.z + 2, second
                    )
                    set_blocks(
                        a.x + 2, a.y - 10, a.z - 2, a.x + 2, a.y, a.z - 2, second
                    )
        ...
        # if the tiles are connected via the z axis
            ...
            if a.z < b.z:
                ...
                if second:
                    set_blocks(
                        a.x + 2, a.y - 30, a.z - 2, a.x + 2, a.y, a.z - 2, second
                    )
                    set_blocks(
                        a.x - 2, a.y - 30, a.z - 2, a.x - 2, a.y, a.z - 2, second
                    )
    ...
    def _tc_pyramid(self):
        ...
        for index in reversed(range(5)):
            ...
            offset += 1
        set_blocks(c.x - 6, c.y - 1, c.z - 6, c.x + 6, c.y - 10, c.z + 6, sandstones[0])

    def _tc_temple(self):
        self._tc_pyramid()
        c = self.grid.center.position
        set_blocks(c.x + 1, c.y + 4, c.z + 1, c.x - 1, c.y + 7, c.z - 1, block.AIR.id)
        for index in range(2):
            set_block(c.x + 1, c.y + 4 + index, c.z + 1, (24, 2))
            set_block(c.x + 1, c.y + 4 + index, c.z - 1, (24, 2))
            set_block(c.x - 1, c.y + 4 + index, c.z + 1, (24, 2))
            set_block(c.x - 1, c.y + 4 + index, c.z - 1, (24, 2))
        set_blocks(c.x + 1, c.y + 6, c.z + 1, c.x - 1, c.y + 6, c.z - 1, (179, 1))
    ...
    def _tc_well(self):
        c = self.grid.center.position
        well = [block.WATER.id, block.LAVA.id]
        set_blocks(c.x + 1, c.y, c.z + 1, c.x - 1, c.y - 6, c.z - 1, block.STONE.id)
        set_blocks(c.x, c.y, c.z, c.x, c.y - 6, c.z, block.AIR.id)
        set_blocks(c.x, c.y - 3, c.z, c.x, c.y - 6, c.z, random.choice(well))
        ...
        set_block(c.x + 1, c.y + 4, c.z, material)
        set_blocks(c.x + 1, c.y + 5, c.z + 1, c.x - 1, c.y + 5, c.z - 1, (126, 5))
    ...
    def _tc_melon_pumpkin(self):
        ...
        c = self.grid.center.position
        set_blocks(c.x + 2, c.y + 1, c.z + 2, c.x - 2, c.y + 5, c.z - 2, choice[0])
        for i in {1, 5}:
        ...
        for i, j in coordinates:
            set_block(c.x + i, get_height(c.x + i, c.z + j) + 1, c.z + j, choice[0])
    ...
    def _tc_pool(self):
        c = self.grid.center.position
        set_blocks(c.x + 3, c.y, c.z + 3, c.x - 3, c.y, c.z - 3, (5, 1))
        set_blocks(c.x + 2, c.y, c.z + 2, c.x - 2, c.y - 3, c.z - 2, block.WATER.id)
        set_blocks(
            c.x + 3,
            c.y + 1,
            c.z + 3,
        ...
        set_blocks(c.x + 2, c.y + 1, c.z + 2, c.x - 2, c.y + 1, c.z - 2, block.AIR.id)
    ...
    def _tc_obelisk(self):
        c = self.grid.center.position
        set_blocks(c.x + 2, c.y, c.z + 2, c.x - 2, c.y + 50, c.z - 2, block.OBSIDIAN.id)

        set_blocks(
            c.x + 1, c.y + 50, c.z + 1, c.x - 1, c.y + 55, c.z - 1, block.OBSIDIAN.id
        )
    ...
    def _tc_fountain(self):
        ...
        for index, element in enumerate([materials["corner"], materials["slab"]]):
            set_block(c.x + 2, c.y + index + 1, c.z + 2, element)
            set_block(c.x + 2, c.y + index + 1, c.z - 2, element)
            set_block(c.x - 2, c.y + index + 1, c.z + 2, element)
            set_block(c.x - 2, c.y + index + 1, c.z - 2, element)
        set_blocks(c.x, c.y - 2, c.z, c.x, c.y + 4, c.z, 251)  # White Concrete
    ...
    def _tc_nether_portal(self):
        ...
        for i in range(-5, 6):
            for j in range(-5, 6):
                set_block(
                    c.x + i, get_height(c.x + 1, c.z + j), c.z + j, selections[index]
                )
                index += 1
        ...
        for i in range(-3, 4):
            for j in range(-3, 4):
                ...
                index += 1
        set_blocks(c.x - 2, c.y + 1, c.z, c.x + 2, c.y + 10, c.z, block.OBSIDIAN.id)
        set_blocks(c.x + 1, c.y + 2, c.z, c.x - 1, c.y + 9, c.z, 90)  # Nether Portal
    ...
    def _tc_end_portal(self):
        ...
        set_blocks(
            c.x - 1, c.y + 1, c.z + 1, c.x + 1, c.y + 1, c.z - 1, 119  # End Portal
        )

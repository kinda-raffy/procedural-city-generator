...
class Cell:
    def __init__(
        ...
        *,
        ...
        self._neighbours: NeighbouringCells = (
            neighbours
            if neighbours
            else dict.fromkeys(NeighbouringCells.__required_keys__)
        )
    ...
    def add_merged_cell(self, other: Cell) -> NoReturn:
        assert (
            other not in self._merged_cells
        ), f"{other!r} is already in connected_cells"
        assert any(
            [other == neighbour for neighbour in self._neighbours.values()]
